...
export interface InvestmentByStatus {
  status: string;
  count: number;
  amount: number;
}

export interface InvestmentMonthlyStats {
  month: string;
  invested: number;
  matured: number;
  withdrawn: number;
...
export interface InvestmentStatisticsData {
  ...
  investmentsByStatus: InvestmentByStatus[];
  monthlyStats: InvestmentMonthlyStats[];
}

// Loan Statistics Types
export interface LoanByType {
  type: string;
  count: number;
  amount: number;
}

export interface LoanByStatus {
  status: string;
  count: number;
  amount: number;
}
...
class DashboardApiClient {
  ...
  constructor() {
    ...
    console.log('Dashboard API Client initialized with baseURL:', baseURL);
    console.log('Token from localStorage:', this.token ? 'Present' : 'Not found');
  ...
  private handleApiError<T>(error: AxiosError): ApiResponse<T> {
    ...
    if (error.response) {
      ...
      const errorMessage = data?.error?.message || data?.message || `Server error: ${error.response.status}`;
  ...
  setToken(token: string): void {
    ...
    console.log('Dashboard API Client token updated');
  }
